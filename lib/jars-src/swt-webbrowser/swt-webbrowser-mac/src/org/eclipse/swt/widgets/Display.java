 ...
 * @noreference This method is not intended to be referenced by clients.
 */
public void internal_dispose_GC (long /*int*/ context, GCData data) {
	if (isDisposed()) SWT.error(SWT.ERROR_DEVICE_DISPOSED);
...
static long /*int*/ performKeyEquivalent(long /*int*/ id, long /*int*/ sel, long /*int*/ arg) {
...
void subclassPanel(id panel, String swtClassName) {
	...
	long /*int*/ proc = performKeyEquivalentCallback.getAddress();
	if (proc == 0) error (SWT.ERROR_NO_MORE_CALLBACKS);
	long /*int*/ cls = OS.object_getClass(panel.id);
	...
	long /*int*/ procPtr = OS.class_getMethodImplementation(cls, OS.sel_performKeyEquivalent_);
	if (procPtr == proc) return;
	long /*int*/ swtPanelClass = OS.objc_getClass(swtClassName);
	if (swtPanelClass == 0) {

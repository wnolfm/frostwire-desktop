...
/**
 * The basics of a ComponentMediator for a Table. Used for: Associating a
 * LimeJTable (TABLE) with a DataLineModel (DATA_MODEL). Associating a
 * JPopupMenu & ButtonRow (BUTTON_ROW) with the DATA_MODEL. Holding common
 * Action/Mouse/ListSelection listeners. (REMOVE_LISTENER, DEFAULT_LISTENER,
 * HEADER_LISTENER, SELECTION_LISTENER) Holding common TableCellRenderers.
 * [static] (PROGRESS_BAR_RENDERER, CHAT_RENDERER) Building a JPanel
 * (MAIN_PANEL) of the LimeJTable, ButtonRow & JPopupMenu. Handling mouse
 * interactions and displaying the appropriate menus. A popup menu if
 * right-click over table. ColumnSelectionMenu if right-click over the header.
 * Sorting the DATA_MODEL if left-click over the header. Refreshing the
 * DATA_MODEL from the RefreshListener's call.
 * 
 * @author Sam Berlin
 */
public abstract class AbstractTableMediator<T extends DataLineModel<E, I>, E extends DataLine<I>, I>
	...
	/**
	 * Variable to the main component displaying this Table. MUST be initialized
	 * in setupConstants()
	 */
	...
	/**
	 * Variable to the LimeJTable for this table. MUST be initialized in
	 * setupConstants()
	 */
	...
	/**
	 * The <tt>Component</tt> containing the <tt>JScrollPane</tt> for the table.
	 */
	...
	/**
	 * Basic constructor that uses a Template Pattern to delegate the setup
	 * functions to individual methods. The following methods are called in the
	 ...
	 * </ul>
	 * Of these, some have are already written as default implementions. The
	 * extending class should call GUIMediator.addRefreshListener(this) if they
	 * want to be a refreshListener, and ThemeMediator.addThemeObserver(this) if
	 * they want to be a ThemeObserver.
	 */
	...
	/**
	 * Sets up Drag & Drop for the table. Default implementation does nothing.
	 * 
	 * This is called prior to addListeners, because D&D wraps all
	 * Mouse[Motion]Listeners behind a proxy, and we don't need to proxy
	 * listeners added from here.
	 */
	...
	/**
	 * Intended for setting the DATA_MODEL and TABLE constants. Optionally,
	 * MAIN_PANEL, BUTTON_ROW, and POPUP_MENU can also be set.
	 */
	protected abstract void setupConstants();

	/**
	 * Assigns the listeners to their slots. This must be done _before_ the
	 * individual components are created, incase any of them want to use a
	 * listener. Extending components that want to build extra listeners should
	 * call super.buildListeners and then build their own. DEFAULT_LISTENER,
	 * SELECTION_LISTENER, HEADER_LISTENER, and REMOVE_LISTENER are created by
	 * default.
	 */
	...
	/**
	 * Adds the listeners to the table. Extending components that want to add
	 * extra listeners should call super.addListeners and then add their own.
	 * DEFAULT_LISTENER, SELECTION_LISTENER and HEADER_LISTENER are added by
	 * default.
	 */
	...
	/**
	 * Sets row heights a little larger than normal, turns off the display of
	 * the grid and disallows column selections.
	 */
	protected void setupTable() {
		...
		TABLE.getTableHeader().addMouseListener(
				new FlexibleColumnResizeAdapter());
	}
	...
	/**
	 * Intended for adding default renderers to the table. Extending components
	 * that want to add extra default renderers should call
	 * super.setDefaultRenderers and then add their own. By default,
	 * PROGRESS_BAR_RENDERER is assigned to ProgressBarHolder.class,
	 * CHAT_RENDERER is assigned to ChatHolder.class, COLOR_RENDERER is assigned
	 * to ColoredCell.class, ICON_RENDERER is assigned to Icon.class, and
	 * ICON_AND_NAME_RENDERER is assigned to IconAndNameHolder.class.
	 */
	...
	/**
	 * Intended for setting up default editors. By default, no editors are
	 * added.
	 */
	...
	/**
	 * Sets up the MAIN_PANEL to have a uniform look among all tables. If the
	 * MAIN_PANEL is not created, nothing will happen.
	 */
	...
	/**
	 * Organizes the table headers so that they're sized correctly, in the
	 * correct order, and are either visible or not visible, depending on the
	 * user's preferences.
	 */
	...
	/**
	 * Add a new DataLine initialized by Object o to the list, tell the
	 * dataModel there was a row inserted, and unselect the first row (to
	 * address the java bug)
	 */
	public void add(I o) {
		add(o, -1);
	}
	...
	/**
	 * Adds a new DataLine initialized by Object o to the list at index i. If
	 * the list is sorted or the insert is beyond the list bounds the add falls
	 * back to the default add(Object o)
	 * 
	 * @param o
	 *            - object to add
	 * @param index
	 *            - index to insert into if the list is not sorted
	 */
	...
	/**
	 * Removes the selection from where the row was added, and puts the focus on
	...
	/**
	 * Removes the row associated with the Object o Delegates to removeRow(int)
	 ...
	 *            - table row to move
	 * @param newLocation
	 *            - location to insert the table row after it had been removed
	 */
	public void moveRow(int oldLocation, int newLocation) {
		if (oldLocation < 0 || oldLocation >= DATA_MODEL.getRowCount())
			throw new IllegalArgumentException("index " + oldLocation
					+ " must be >= 0 and < TABLE.getRowCount()");
		if (newLocation < 0 || newLocation >= DATA_MODEL.getRowCount())
			throw new IllegalArgumentException("add index " + newLocation
					+ " must be >= 0 and < TABLE.getRowCount()");
	...
	/**
	 * Implements RefreshListener Wraps the doRefresh call so that extending
	 * classes can maintain the resort & isShowing checks. Extending classes
	 * should NOT override this. Instead, they should override doRefresh.
	 */
	public void refresh() {
	...
	/**
	 * Exists for extending classes to overwrite. Mostly used for updating the
	 * values of buttons/menu-items With the return value of the DATA_MODEL's
	 * refresh.
	 */
	...
	/**
	 * Removes all selected rows from the list and fires deletions through the
	 * dataModel
	 * 
	...
	/**
	 * Creates a new ColumnSelectionMenu JPopupMenu. If the table wants to use a
	 * custom popup menu, override this method and do something different.
	 */
	...
	public void handleHeaderPopupMenu(Point p) {
		createColumnSelectionMenu().show(TABLE.getTableHeader(), p.x + 1,
				p.y - 6);
	}
	...
	/**
	 * Sorts the DATA_MODEL and maintains selections in the TABLE. If
	 * columnToSort is -1, it will resort the current column. Otherwise it will
	 * sort columnToSort.
	 */
	...
	/**
	 * Abstract method for creating a right-click popup menu for the table. If
	 * an implemention does not support a right-click popup menu, it should
	 * return <tt>null</tt>.
	 * 
	...
	protected final class Resorter implements Runnable {
		...
		/**
		 * To save processor usage, only resort those tables that are currently
		 * showing, are sorting in real time, and are not actively being sorted.
		 */
		...
		/**
		 * Iff the data model needs resorting (meaning the selected column is a
		 * 'dynamic' column), then do the resort. We need to remember what was
		 * selected because resorting will invalidate the selections. We cannot
		 * do it while the table is editing because editing overrides what is
		 * displaying in that cell, making the user think they are still editing
		 * the correct line, when infact it has moved.
		 */
		public void run() {
			try {
	 ...
	 * 
	 * Handles comparison uniquely for 'native' types. We want to compare
	 * strings by lowercase comparison Note that non-integer comparisons must
	 * specifically check if the difference is less or greater than 0 so that
	 * rounding won't be wrong. Of the native types, we check 'Integer' first
	 * since that's the most common, Boolean, then Double or Float, and finally,
	 * the rest will be caught in 'Number', which just uses an int comparison.
	 */

...
public class MagnetPlugin2 implements Plugin {
    ...
    public void initialize(PluginInterface _plugin_interface) {
        ...
        MagnetURIHandler.getSingleton().addListener(new MagnetURIHandlerListener() {
            ...
            public byte[] download(final MagnetURIHandlerProgressListener muh_listener, final byte[] hash, final String args, final InetSocketAddress[] sources, final long timeout)
            ...
            throws MagnetURIHandlerException {
                ...
                return (MagnetPlugin2.this.download(new MagnetPluginProgressListener() {
    ...
    public byte[] download(MagnetPluginProgressListener listener, byte[] hash, String args, InetSocketAddress[] sources, long timeout)
    ...
    private byte[] downloadSupport(final MagnetPluginProgressListener listener, final byte[] hash, final String args, final InetSocketAddress[] sources, final long timeout)
    ...
    public static void main(String[] args) throws Exception {
        ...
        final TOTorrentMetadata torrent = new TOTorrentMetadata(ByteFormatter.decodeString("54dec3e7b1169fad5587d5a9e30fafa92097eab7"), "54dec3e7b1169fad5587d5a9e30fafa92097eab7", trackers);
        ...
        peerManager.stopAll();
        tracker_client.stop(false);
    }

    private static class MetadataTrackerAnnouncerListener implements TRTrackerAnnouncerListener {
    ...
    private static class MetadataDiskManager implements DiskManager {
    ...
    private static class MetadataTrackerAnnouncerDataProvider implements TRTrackerAnnouncerDataProvider {
    ...
    private static class MetadataPeerListener implements PEPeerListener {

        private static boolean metadataRequested;

        @Override
        public void stateChanged(PEPeer peer, int new_state) {
            if (new_state == (PEPeer.TRANSFERING + 1) && peer instanceof PEPeerTransportProtocol && !metadataRequested) {
                final PEPeerTransportProtocol transport = (PEPeerTransportProtocol) peer;
                new Thread() {
                    public void run() {
                        try {
                            Thread.sleep(2000);
                        ...
                        if (transport.supportsMetadataRequest() && !metadataRequested) {
                            metadataRequested = true;
                            transport.sendMetadataRequest(0);
                        }
                    };
                }.start();
            }
    ...
    private static class MetadataPeerManagerRegistration implements PeerManagerRegistration {
    ...
    private static class MetadataPeerManagerAdapter implements PEPeerManagerAdapter {
        ...
        public PeerManagerRegistration getPeerManagerRegistration() {
            return new MetadataPeerManagerRegistration();
        }
